    ...
    <>
          ...
          {nowPlayingMovies.map((movie) => (
            <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300`} onClick={() => setSelectedMovie(movie.id)}>
              ...
              {selectedMovie === movie.id && (
                <div className='absolute top-2 right-2 bg-primary h-6 w-6 rounded flex justify-center items-center'>
      ...
      <div className='mt-8'>
        ...
        <div className='inline-flex items-center gap-2 border border-gray-600 px-3 rounded-md'>
          ...
          <input type="number" min={0} value={showPrice} placeholder='Enter show price' className='outline-none' onChange={(e) => setShowPrice(e.target.value)} />
        </div>
      </div>
      ...
      <div className='mt-6'>
        ...
        <div className='inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg'>
          ...
          <button onClick={handleDateTimeAdd} className='bg-primary/80 px-3 py-2 text-sm rounded-lg cursor-pointer hover:bg-primary text-white'> Add Time</button>
        </div>
      </div>
